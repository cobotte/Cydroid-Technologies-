/**
 * CYDROID TECHNOLOGIES — Book Demo API Route
...
import { validateBookDemoForm } from "@/utils/validation";
import {
  bookDemoRateLimiter,
  getClientIP,
  isOversizedRequest,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rateResult = bookDemoRateLimiter.check(ip);
    ...
    if (body.website_honey && String(body.website_honey).trim() !== "") {
      logSecurityEvent({ event: "HONEYPOT_TRIGGERED", ip, path });
      // Return fake success to confuse bots
      return NextResponse.json({ success: true, referenceNumber: "COB-00000" }, { status: 200 });
    }

    // ── Guard 6: Schema validation (includes SQLi + XSS detection) ────────────
    const validation = validateBookDemoForm(body);
    if (!validation.success) {
      if (validation.errors?._security) {
        logSecurityEvent({
          ...
          ip,
          path,
          detail: "XSS or SQLi pattern in book-demo form",
        });
      }

      const publicErrors = { ...validation.errors };
      delete publicErrors._security;
    ...
    // ── Success ───────────────────────────────────────────────────────────────
    const referenceNumber = generateReference("COB");
    ...
    // TODO: Replace with CRM/email integration
    console.warn("=== NEW DEMO REQUEST ===");
    console.warn("Reference:", referenceNumber);
    console.warn("Name:", data.fullName);
    console.warn("Email:", data.emailAddress);
    console.warn("Project Types:", data.projectTypes.join(", "));
    console.warn("Budget:", data.budgetRange);
    ...
    return NextResponse.json(
      {
        success: true,
        referenceNumber,
        message: "Your demo request has been received. We will be in touch within 24 hours.",
      },
      { status: 200 }
    );

  } catch (err) {
    console.error("[BOOK-DEMO API ERROR]", err instanceof Error ? err.message : "Unknown error");
